...
copy_file_from() {
  ...
  ui_print "Copy libmemtrack.so to libmemtrack_real.so"
  cp "$FULL_PATH" "$MODPATH/system/$FROM_PATH/libmemtrack_real.so" || abort "- Copy $REPLACED_PATH failed."
  return 0
}
